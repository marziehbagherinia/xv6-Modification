...
pagefault_handler(struct trapframe *tf)
{
  ...
  mmapped_space *mmap_node = curproc->space_head;
  ...
  while(mmap_node)
  {
    if ((uint)(mmap_node->start_addr) <= fault_addr && 
        (uint)(mmap_node->start_addr + mmap_node->length) > fault_addr)
    { 
      if ((mmap_node->prot & PROT_WRITE) || !(tf->err & T_PGFLT_W)) 
        valid = 1;
      break;
    }
    else
  ...
  if (valid == 1)
  {
    ...
    if (mmap_node->region_type == MAP_SHARED)
    {
